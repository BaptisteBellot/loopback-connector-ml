...
MarkLogic.prototype.all = function all(model, filter, options, callback) {
  ...
  if (filter.where && filter.where._uri) {
    ...
    self.db.documents.query(model).result().then(function(response) {

      processResponse(response);
    });
  } else {
    console.log('aaa', query);
    if(query.model && query.property) query = {};
    var criteria = self.qb.where(self.qb.byExample(query)).withOptions({categories: ['content', 'metadata']}).slice(0,10000);

      console.log('criteria', criteria);
    self.db.documents.query(criteria).result().then(function(response) {
  ...
  function processResponse(cursor) {
      ...
      cursor.forEach(function(entry) {
        ...
        if(entry.content.created) {
          console.log(new Date(entry.content.created));
          entry.content.created =  new Date(entry.content.created);
        }
        if(entry.content.time) {
          console.log(new Date(entry.content.time));
          entry.content.created =  new Date(entry.content.time);
        }
        if(entry.collections) {
          if(entry.collections.indexOf(model) > -1){
    ...
    if(filter.skip || filter.limit) {
      results = results.slice((filter.skip || 0), (filter.skip || 0)+(filter.limit || 100000))
    }
    if(filter.order) {
      let rawParam = filter.order.split(' ');
      let targetfield = rawParam[0];
      let direction = rawParam[2];
      results.sort((itemA,itemB)=>{
        switch(direction) {
          case 'DESC':
                return itemA[targetfield] > itemB[targetfield] ? 1:-1;
          default:
               return itemA[targetfield] > itemB[targetfield] ? -1:1;
        }
